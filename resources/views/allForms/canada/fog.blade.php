            ...
            <table style="width:100%; border-left:3px solid#8a898a; border-right:3px solid#8a898a;">
                ...
                <tr>
                    <td class="padding" id="colsborder" colspan="2"
                        style="border:3px solid #8a898a; text-align: left; padding-left:20px; border-top:none; text-transform:capitalize; border-bottom:none;font-size:13px;">
                        {{ $requestData['emp_id'] }}
                    </td>
                    <td class="padding"
                        style="border:3px solid #8a898a; text-align:center; border-top:none; border-bottom:none;font-size:13px;"
                    ...
                    <td class="padding" colspan="2"
                        style="border:3px solid #8a898a; text-align:center;border-top:none; border-bottom:none;font-size:13px;">{{ $requestData['check_number'] ?? '' }}
                    </td>
                </tr>
            </table>
            <section>
                <div class="row"
                    style=" background-color:#d6d6d6; border-left:2px solid #8a898a;border-right:3px solid#8a898a;">
                    <div class="column1">
                        <table class="colortable" style="width:100%;">
                            <thead id="colourborder">
                                ...
                                <th class="padding" style="font-size:13px;border-right:3px soliddarkgrey;">CURRENT TOTAL</th>
                            </thead>
                            <tbody style="">
                                @foreach ($requestData['earning'] as $key => $earn)
                                    @if(strlen($earn) > 0)
                                        <tr style="">
                                            ...
                                            <td style="font-size:13px;"> @if($requestData['rate'][$key] != 0.00) {{ number_format($requestData['rate'][$key], 2) ?? '' }}@endif</td>
                                            <td style="text-align: right;padding-right:8px;font-size:13px;">{{ $requestData['hours'][$key] }}.00 </td>
                                            <td style="text-align: right;padding-right:20px;font-size:13px; ">{{ $requestData['currency'] }}{{ number_format($requestData['total'][$key], 2) }}</td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
                    ...
                    <div class="column2">
                        <table class="colortable" style="width:100%;">
                            <thead id="colourborder">
                                <th class="padding" style="text-align: left;padding-left:15px;font-size:13px;border-left:3px solid darkgrey; ">DEDUCTIONS</th>
                                <th class="padding" style="font-size:13px;">CURRENT TOTAL</th>
                                <th class="padding" style="font-size:13px;">YEAR TO DATE</th>
                            </thead>
                            <tbody id="" style=" border-left:3px solid#8a898a;font-size:13px;">
                                @foreach ($requestData['taxes'] ?? [] as $key => $taxes)
                                    @if(strlen($taxes) > 0)
                                        <tr>
                                            <td style="font-size:13px; padding-left:14px;" id="fica">{{ $taxes }}</td>
                                            <td style="text-align: center;font-size:13px;">{{ $requestData['currency'] }}{{ number_format($requestData['taxes_rate'][$key], 2) }}</td>
                                            <td style="text-align: right;padding-right:5px;font-size:13px;">{{ $requestData['currency'] }}{{ number_format($requestData['taxes_ytd'][$key], 2) }}</td>
                                        </tr>
                                    @endif
                                @endforeach
                ...
                <div class="row" style=" border:3px solid#8a898a;">
                    <div class="column1"style="width:50.3%;">
                        <table id="bottomtable" style="width:100%; background-color:white;">
                            <tr class="">
                                <th id="cols" class="head1" style="width: 20%;">YTD GROSS</th>
                                <th id="cols" class="head1" style="width: 25%;">YTD DEDUCTIONS</th>
                                <th id="cols" class="head1"style="padding-left:20px;width: 22%;">YTD NET PAY</th>
                            </tr>
                            <tr class=" ytd">
                                <td id="cols" class="head2" style="width: 20%;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['ytd_gross_total'], 2) }} </td>
                                <td id="cols" class="head2" style="width: 25%;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['ytd_deduction_tax'], 2) }} </td>
                                <td id="cols" class="head2" style="width: 22%;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['total_ytd_net_pay'], 2) }} </td>
                            </tr>
                        </table>
                    </div>
                    <div class="column2" style="width:49.3%;">
                        <table id="bottomtable" style="width:100%; background-color:white;">
                            <tr class="">`
                                <th id="cols" class="head1" style="width: 20%;">CURRENT TOTAL</th>
                                <th id="cols" class="head1" style="width: 17%;">DEDUCTIONS</th>
                                <th id="cols" class="head1" style="border-right: none !important;width: 15%; ">NET PAY
                                </th>
                            </tr>
                            <tr class=" ytd">
                                <td id="cols" class="head2" style="width: 20%;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['period_gross_total'], 2) }} </td>
                                <td id="cols" class="head2" style="width: 17%;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['deduction_tax'], 2) }} </td>
                                <td id="cols" class="head2" style="width: 15%;" style="border-right: none !important; font-family: DejaVu Sans, sans-serif;"><span style="font-family: 'DejaVu Sans', sans-serif;">{{ $requestData['currency'] }}</span>{{ number_format($requestData['total_net_pay'], 2) }} </td>
                            </tr>
                        </table>
                    </div>
                </div>
