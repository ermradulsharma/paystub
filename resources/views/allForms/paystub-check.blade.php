<!DOCTYPE html>
<html lang="en">

<head>
    <title>paystubs-check</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<style>
    th {
        text-align: left;
    }

    table {
        font-size: 13px;
    }

    .two-col {
        -webkit-column-count: 2;
        /* Chrome, Safari, Opera */
        -moz-column-count: 2;
        /* Firefox */
        column-count: 2;
    }
</style>

<body>

    <table>
        <tr>
            <td>CO</td>
            <td>FILE</td>
            <td>DEPT.</td>
            <td>CLOCK VCHR</td>
        </tr>
        <tr>
            <td>MPS</td>
            <td>001755</td>
            <td>201094</td>
            </td>
            <td>NO.000000422572</td>
        </tr>
    </table>
    <table style="width: 100%;">
        <tr>
            <td style="font-weight: 800;">Paystubs,Inc</td>
            <td></td>
            <th>Earnings Statement</th>
        </tr>
    </table>
    <table style="width: 100%;">
        <tr>
            <td>1717 CALIFORNIA LN</td>
            <td>Period Beginning:</td>
            <td>03/28/23</td>
        </tr>

        <tr>
            <td>JAMAICA NY, 11433</td>
            <td>Period Ending:</td>
            <td>04/03/23</td>
        </tr>

        <tr>
            <td>USA</td>
            <td>Pay Date:</td>
            <td>04/04/23</td>
        </tr>
    </table>
    <table style="width: 100%; margin-top: 60px;">
        <tr>
            <td>Social Security Number:</td>
            <td>***_**_1234</td>
            <td style="font-weight: 800;">James Moore</td>
        </tr>

        <tr>
            <td>Marital Status:</td>
            <td>Single</td>
            <td>2355 AUSTIN ST</td>
        </tr>

        <tr>
            <td>Expectations/Allowances:</td>
            <td>0</td>
            <td>JAMAICA, NY. 31314, USA</td>
        </tr>
    </table>
    <section>
        <table style="width: 100%; margin-top: 60px;">
            <tr>
                <th>Earning</th>
                <th style="width: 15%;">Rate</th>
                <th>Hours</th>
                <th>This Period</th>
                <th >Year-to-date</th>
                <th style="border-bottom: 2px solid black;">Important Note</th>
            </tr>
            <tr>
                <td>Regular</td>
                <td>10</td>
                <td>40</td>
                <td>400</td>
                <td>5.600.00</td>
                <td style="font-size: 10px;">Company telephone No: 12345678456</td>
            </tr>
            <tr>
                <td>Overtime</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td></td>
            </tr>
            <tr>
                <td>Holiday</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td></td>
            </tr>
            <tr>
                <td>Vaction</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td></td>
            </tr>
            <tr>
                <td>Bonus</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td></td>
            </tr>
            <tr>
                <td>float</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td>0.00</td>
                <td></td>
            </tr>
            <tr style="font-weight: 800;">
                <td></td>
                <td></td>
                <td>GROSS PAY</td>
                <td>400.00</td>
                <td>5.600.00</td>
            </tr>
        </table>
    </section>
   






    <table style="width:100%;margin-top:30px;">
        <thead>
            <th>Deductions</th>
            <th colspan="4">Statuory</th>
        </thead>
        <tr>
            <td></td>
            <td style=" width: 35%;">Medicare Tax</td>
            <td style="width: 17%;">5.80</td>
            <td>81.20</td>
        </tr>

        <tr>
            <td></td>
            <td>Social security Tax</td>
            <td>5.80</td>
            <td>81.20</td>
        </tr>
        <tr>
            <td></td>
            <td>Fedral income Tax</td>
            <td>5.80</td>
            <td>81.20</td>
        </tr>
        <tr>
            <td></td>
            <td>State Income Tax</td>
            <td>5.80</td>
            <td>81.20</td>
        </tr>

        <tr>
            <td></td>
            <th>Total Deductions</th>
            <th>5.80</th>
            <th>81.20</th>
        </tr>

        <tr>
            <th></th>
            <th>NET PAY</th>
            <th>5.80</th>
            <th>81.20</th>
        </tr>
    </table>



    <table class="container" style=" margin-top:100px;padding: 0 0px 0px 0px;width:100%; ">
        <div class="row" style="display: flex; display: flex;justify-content: space-between;padding: 0px 14px;">
            <div style="width: 50%;float:left;">
                <h6 style="font-size: 17px; margin-bottom: 0;">Paystubs Company, INC</h6>
                <p style="font-size: 10px; margin: 0;">55 NORTHERN BLVD</p>
                <P style="font-size: 10px; margin: 0;">SUIT 206</P>
                <P style="font-size: 10px; margin: 0;">GREAT NECK, FAIRFIELD 45014</P>
                <p style="font-size: 10px;">Pay To: <span style="font-weight:800;">ERZA MOORE</span>
                </p>
            </div>
            <div style="width: 50%;float:right">
                <h6 style="font-size: 17px; margin-bottom: 0;">Advice Number: <span>00000422598</span>
                </h6>
                <br>
                <br>
                <P style="font-size: 10px;margin: 0;">
                    <span style="font-weight:800;">Check Nuumber:</span> 1775
                </P>
                <P style="font-size: 10px;margin: 0;">
                    <span style="font-weight:800;">Date:</span> 1775
                </P>
            </div>
        </div>
       
      
        
    </table>


    <table style="width: 100%; margin: 100px 0 0 auto;">
        <tr style="border-bottom: 1px solid black;">
        <td colspan="4"></td>
        <td style="">Deposite to the Account off</td>
            <td style="text-align: right;">Amount</td>
            <td style="text-align: right;">Transit ABA </td>
            <td style="text-align: right;">Account number</td>
        </tr >
       
        <tr>
        <td colspan="4"></td>
        <td style="">Erza Moore</td>
            <td style="text-align: right;">XX567</td>
            <td style="text-align: right;">XXX567</td>
            <td style="text-align: right;">XXXX5677</td>
        </tr>
    </table>
</body>

</html>